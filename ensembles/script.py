...
"""
def get_subs(predfile, subfile):
    subs = np.hstack([np.array(pd.read_csv(subfile)[LABELS]) for file in subfiles])
    oofs = np.hstack([np.array(pd.read_csv(predfile)[LABELS]) for file in predfiles])
    return subs, oofs
...
if __name__ == "__main__":
    ...
    predfiles = ['prediction_DPCNN.pkl', 'prediction_RNN_Capsule.pkl',
 prediction_RNN_LSTM.pkl',
 preds_lgbm.csv']
    subfiles = 
    subs, oofs = get_subs(predfiles, subfiles)
